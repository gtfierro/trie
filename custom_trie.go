...
// CustomTrie is a trie of paths with string keys and interface{} values.

// CustomTrie is a trie of string keys and interface{} values. Internal nodes
// have nil values so stored nil values cannot be distinguished and are
// excluded from walks. By default, CustomTrie will segment keys by forward
...
func (trie *CustomTrie) Get(key string) interface{} {
	node := trie
...
func (trie *CustomTrie) Put(key string, value interface{}) bool {
	node := trie
	for part, i := trie.segmenter(key, 0); ; part, i = trie.segmenter(key, i) {
...
func (trie *CustomTrie) Delete(key string) bool {
	var path []customNodeStr // record ancestors to check later
	node := trie
	for part, i := trie.segmenter(key, 0); ; part, i = trie.segmenter(key, i) {
		path = append(path, customNodeStr{part: part, node: node})
		node = node.children[part]
		if node == nil {
			// node does not exist
			return false
...
func (trie *CustomTrie) Walk(walker WalkFunc) error {
	return trie.walk("", walker)
}

// PathTrie node and the part string key of the child the path descends into.
type customNodeStr struct {
	node *CustomTrie
	part string
}

func (trie *CustomTrie) walk(key string, walker WalkFunc) error {
	if trie.value != nil {
...
func (trie *CustomTrie) isLeaf() bool {
